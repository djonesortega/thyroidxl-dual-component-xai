...
FINAL_STATUS = "THYROIDXL_EFFICIENTNETB3_FINAL_OFFICIAL_TRAIN_REFIT_ONEFOLD_SELECTED"
DEVELOPMENT_STATUS = "THYROIDXL_EFFICIENTNETB3_ONEFOLD_DEVELOPMENT_SELECTED"
...
ROOT = project_root()
EFFICIENT_MODELS_ROOT = ROOT / "Models" / "EfficientNetB3"
...
def find_final_checkpoint_and_manifest():
    if not EFFICIENT_MODELS_ROOT.is_dir():
        raise FileNotFoundError(
            f"Missing EfficientNet-B3 model directory: {EFFICIENT_MODELS_ROOT}"
        )

    checkpoint_matches = []

    for path in EFFICIENT_MODELS_ROOT.rglob("*.pt"):
        try:
    ...
    if int(manifest.get("training_images", EXPECTED_TRAIN_IMAGES)) != EXPECTED_TRAIN_IMAGES:
        raise RuntimeError("Final EfficientNet-B3 manifest training-image count mismatch.")
    if int(manifest.get("training_patients", EXPECTED_TRAIN_PATIENTS)) != EXPECTED_TRAIN_PATIENTS:
        raise RuntimeError("Final EfficientNet-B3 manifest training-patient count mismatch.")
...
# ---------------------------------------------------------------------
# Exact EfficientNet-B3 + segmentation decoder reconstruction
# ---------------------------------------------------------------------
...
class EfficientNetB3MultiscaleDiceBCE(nn.Module):
    """Exact reconstruction of the reviewed EfficientNet-B3 multitask network."""
    def __init__(
        self,
        ...
        if expected_feature_channels is not None:
            ...
            if inferred != expected:
                raise RuntimeError(
                    "Installed timm EfficientNet-B3 architecture does not match "
        ...
        if tuple(final_feature.shape[-2:]) != expected_final:
            raise RuntimeError(
                "Unexpected final EfficientNet-B3 feature resolution: "
                f"{tuple(final_feature.shape)}"
            )

        self.decoder_bottleneck = ConvBNAct(inferred["final"], 160)
        self.decoder_skip32 = SkipFusionBlock(160, inferred["skip32"], 112)
        self.decoder_skip64 = SkipFusionBlock(112, inferred["skip64"], 80)
        self.decoder_skip128 = SkipFusionBlock(80, inferred["skip128"], 48)
        self.decoder_up256 = UpsampleRefineBlock(48, 24)
        self.decoder_up512 = UpsampleRefineBlock(24, 12)
        self.segmentation_output = nn.Conv2d(12, 1, kernel_size=1)
    ...
    def _encode(self, image):
        ...
        for target in (32, 64, 128):
            ...
            if not candidates:
                ...
                raise RuntimeError(
                    f"No EfficientNet-B3 skip feature at {target}x{target}. "
...
def load_model(
    ...
    model = EfficientNetB3MultiscaleDiceBCE(
...
def original_geometry_segmentation_metrics(
    ...
    """
    Compute EfficientNet-B3 auxiliary-segmentation metrics in ORIGINAL ultrasound
    coordinates.
...
"""
Canonical FINAL EfficientNet-B3 evaluation for the reviewed ThyroidXL notebook.
...
def run_final_inference(
    ...
    with torch.inference_mode():
        for batch in tqdm(
            loader,
            desc="EfficientNet-B3 official held-out test",
            unit="batch",
        ):
...
def save_figures(
    ...
    ax.set_title(
        "EfficientNet-B3 auxiliary segmentation — held-out Dice (original geometry)"
    )
    fig.tight_layout()
    fig.savefig(
        OUT
...
def main():
    ...
    print(
        ...
        "after inverse-mapping the EfficientNet-B3 probability map."
    )
    print()
    ...
    # These do not change any operating rule: EfficientNet-B3 primary remains fixed 0.5.
    ...
    summary = {
        "status": (
            "THYROIDXL_EFFICIENTNETB3_FINAL_PUBLICATION_EVALUATION"
        ),
        "dataset": "ThyroidXL",
...
MODEL_RESULT_DIR = 'EfficientNetB3'
MODEL_XAI_TAG = 'EFFICIENTNETB3'
