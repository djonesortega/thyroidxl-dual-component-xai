...
ROOT = project_root()
MOBILE_MODELS_ROOT = ROOT / "Models" / "MobileNetV3"
...
def find_final_checkpoint_and_manifest():
    if not MOBILE_MODELS_ROOT.is_dir():
        raise FileNotFoundError(
            f"Missing MobileNet model directory: {MOBILE_MODELS_ROOT}"
        )

    checkpoint_matches = []

    for path in MOBILE_MODELS_ROOT.rglob("*.pt"):
        try:
    ...
    if int(manifest.get("training_images", EXPECTED_TRAIN_IMAGES)) != EXPECTED_TRAIN_IMAGES:
        raise RuntimeError("Final MobileNet manifest training-image count mismatch.")
    if int(manifest.get("training_patients", EXPECTED_TRAIN_PATIENTS)) != EXPECTED_TRAIN_PATIENTS:
        raise RuntimeError("Final MobileNet manifest training-patient count mismatch.")
...
# ---------------------------------------------------------------------
# Exact MobileNetV3 + segmentation decoder reconstruction
# ---------------------------------------------------------------------
...
class MobileNetV3MultiscaleDiceBCE(nn.Module):
    def __init__(
        ...
        # pretrained=False is correct for evaluation because the saved
        # state_dict is loaded immediately afterwards.
        ...
        if expected_feature_channels is not None:
            expected = {
                str(key): int(value)
                for key, value in expected_feature_channels.items()
            }
            if inferred != expected:
                raise RuntimeError(
                    "Installed timm architecture does not match "
                    f"checkpoint metadata. Expected={expected}, "
                    f"inferred={inferred}"
                )
    ...
    def _encode(self, image):
        ...
        for target in (32, 64, 128):
            ...
            if not candidates:
                ...
                raise RuntimeError(
                    f"No MobileNetV3 skip feature at {target}x{target}. "
...
def load_model(
    ...
    # The reviewed DEVELOPMENT checkpoint intentionally did not persist
    # drop_rate, so use the notebook's frozen value 0.20 as its fallback.
    model = MobileNetV3MultiscaleDiceBCE(
        ...
        expected_feature_channels=(
            checkpoint.get("feature_channels")
        ),
    ).to(device)
...
def original_geometry_segmentation_metrics(
    ...
    """
    Compute MobileNet auxiliary-segmentation metrics in ORIGINAL ultrasound
    coordinates.
...
"""
Canonical FINAL MobileNet evaluation for the reviewed ThyroidXL notebook.
...
def run_final_inference(
    ...
    with torch.inference_mode():
        for batch in tqdm(
            loader,
            desc="MobileNet official held-out test",
            unit="batch",
        ):
...
def save_figures(
    ...
    ax.set_title(
        "MobileNet auxiliary segmentation — held-out Dice (original geometry)"
    )
    fig.tight_layout()
    fig.savefig(
        OUT
...
def main():
    ...
    print(
        ...
        "after inverse-mapping the MobileNet probability map."
    )
    print()
    ...
    # These do not change any operating rule: MobileNet primary remains fixed 0.5.
...
MODEL_DISPLAY_NAME = 'MobileNetV3'
MODEL_RESULT_DIR = 'MobileNet'
MODEL_XAI_TAG = 'MOBILENETV3'
...
# Locked MobileNet publication XAI configuration:
#   Grad-CAM   -> 16x16
#   Grad-CAM++ -> 16x16 (restored original layer)
#   Layer-CAM  -> 32x32 + 16x16
...
def xai_main():
    ...
    print("MobileNet XAI configuration: locked final method-specific configuration")
    print()
    ...
    freeze = {
        ...
        "configuration_source": "locked_mobile_final_configuration_gradcam16_gradcampp16_layercam32plus16",
    ...
    print("No further XAI tuning is performed in this run.")
    print()
    ...
    print("MobileNet configuration used: Grad-CAM 16; Grad-CAM++ 16; Layer-CAM 32+16.")
